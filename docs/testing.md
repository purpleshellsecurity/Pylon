...
- **An unconfirmed table is labelled, and the label is the point.** `design` says
  next to each detection how much its table name is worth: confirmed (the scanned
  workspace holds data in it), NOT confirmed, or unchecked when no scan document
  was present. A "NOT confirmed" line is not a fault to report — it is the tool
  telling you it could not prove the tenant writes to that table, which on a
  quiet resource is the honest answer.
