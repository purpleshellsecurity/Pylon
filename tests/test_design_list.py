...
def test_a_category_slice_reports_its_own_vocabulary_not_the_directory():
    ...
    assert "82 known operations" in block
    assert "922" not in block


def test_the_hint_reads_before_the_list(capsys):
    """Two streams arriving at one terminal put the hint after the values,
    where it read as a footer the command had trailed off into. It stays on
    stderr -- `pylon design list | fzf` must receive only values -- and is
    printed first."""
    import argparse

    from pylon import cli

    cli._design_list(argparse.Namespace(source=None, target=[]))
    out = capsys.readouterr()
    assert "pylon design list <value>" in out.err
    assert "pylon design list <value>" not in out.out
    # Every line of stdout is a value someone can type.
    assert all(line and not line.startswith(" ")
               for line in out.out.splitlines()), out.out
