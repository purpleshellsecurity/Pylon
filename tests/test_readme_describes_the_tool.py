...
# Where a module may live. Deliberately NOT `root.rglob(name)`, which was the
# first version of this: rglob from the repo root walks `build/`, `dist/` and
# `.venv/` too, so a module DELETED from `src/` still answers "present" for as
# long as a stale build tree holds a copy. This checkout had exactly that --
# `build/lib/pylon/report_detections.py`, dated 2026-09-12, from a build made
# while the module still existed; it was deleted from src/ on 2026-09-15.
#
# `build/` is gitignored, so CI's fresh checkout does not have it and this check
# was STRICTER in CI than on the machine where the prose gets written. That is
# the wrong way round: the author sees green and the gate fires later, on
# somebody else's run, about a sentence they did not write.
_SOURCE_DIRS = ("src", "tests", "scripts")


def _on_disk(root, name: str) -> bool:
    """True when `name` is a file in this project's own source, not in build output."""
...
def test_every_module_the_readme_names_exists():
    ...
    missing = sorted(name for name in named if not _on_disk(root, name))
    assert not missing, (
...
# How to invoke the CLI from a test, without assuming anything about PATH.
#
# This ran bare `pylon`, which resolves only when the virtualenv is ACTIVATED.
# CI activates it (`uv run pytest`), so the suite was green everywhere it ran --
# and a Windows box invoking pytest by absolute path, which is the obvious thing
# to do, failed all thirteen examples with an empty help text and no hint why.
# Reproduced on macOS by clearing PATH: the same thirteen, so it was never about
# Windows.
#
# `sys.executable -m pylon.cli` needs no PATH, no console script, and no
# activation, and it runs the interpreter that is already running the tests --
# which is also the only way to be sure it is the same install.
_PYLON = (sys.executable, "-m", "pylon.cli")
...
def test_every_flag_in_an_example_exists(example):
    ...
    if not verb:
        pytest.skip("no verb")
    helptext = subprocess.run([*_PYLON, *verb, "--help"], capture_output=True,
...
def _env_vars_in_code(path) -> set[str]:
    """Variables the code READS, ignoring names that only appear in a comment.

    A plain regex over the file cannot tell a use from a mention, and that has
    cost something three times in this repo now -- twice on the release scan
    matching its own documentation of a rule, and here. `config.py` carries a
    comment recording that `PYLON_MAX_COST` and `PYLON_MAX_TOKENS` were removed
    because nothing read them; the regex counted that comment as a read and
    demanded the README document two variables that do not exist.

    Comments are dropped, so the note can stay where it is useful. Strings are
    NOT dropped: `os.environ["PYLON_X"]` is a string, and it is the read.
    """
    import io
    import tokenize
    ...
    try:
        ...
        body = src            # unparseable: fall back to the whole file
    return _env_vars(body)
...
def test_no_test_invokes_the_console_script_by_bare_name():
    """A test that runs bare `pylon` only works when the virtualenv is
    ACTIVATED. CI activates it, so this passed everywhere it ran -- and a
    Windows box invoking pytest by absolute path, the obvious thing to do,
    failed thirteen README examples with an empty help text and no hint why.
    Reproduced on macOS by clearing PATH; it was never about Windows.

    Use `sys.executable -m pylon.cli`: no PATH, no console script, no
    activation, and it is provably the same install that is running the tests.
    """
    import pathlib
    import re
    ...
    for f in sorted(pathlib.Path(__file__).parent.glob("test_*.py")):
        for i, line in enumerate(f.read_text(encoding="utf-8").splitlines(), 1):
            if re.search(r"""subprocess\.(run|Popen|check_\w+)\(\s*\[\s*["']pylon["']""", line):
                bad.append(f"{f.name}:{i}")
    assert bad == [], (
        "these invoke the console script by bare name, which needs the "
        f"virtualenv on PATH: {bad}")
