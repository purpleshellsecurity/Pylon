...
def test_each_run_starts_from_a_zero_meter(monkeypatch):
    ...
    # `runnable()`, not `pylon`. The harness goes through the accessor that
    # owns the `@workflow` shape change between agent_framework 1.13 and 1.18.
    _workflow = type("W", (), {"run": staticmethod(_fake_run)})
    monkeypatch.setattr(mod, "runnable", lambda: _workflow)
