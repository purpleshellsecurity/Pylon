...
# A technique id is not unique within a table. `T1685.002` on AzureActivity has
# three separate claims -- diagnostic settings, Key Vault, and SQL auditing --
# and a lookup keyed by technique alone kept whichever came last. Twelve ids on
# that table collide the same way, one discarding four alternatives.
#
# It was a REGRESSION EXPOSED BY A FIX. Before ids were normalised onto their
# current spellings the run said T1685.002 while the catalogue said T1562.008,
# the lookup missed, and the section was absent. Making the two sides agree
# turned a missing section into a confidently wrong one, which is worse:
