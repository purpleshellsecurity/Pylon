...
def _section(html, eyebrow):
    """One section by name, whatever step it is numbered.

    The eyebrows carry their position in the chain now ("Step 3 · Volume and
    cost"), and these tests are about what a section SAYS, not where it sits.
    Matching the bare name with an optional prefix keeps them from breaking
    every time the running order changes, which is the thing being iterated on.
    """
    found = re.search(
        rf'<div class="eyebrow">(?:[^<]*?· )?{eyebrow}</div>.*?</section>',
        html, re.S)
...
def test_a_commitment_tier_is_named_because_it_is_bought_below_list(monkeypatch):
    ...
    unread = _section(report.build(_doc(workspace_plan={
        ...
        "retention_days": 90}), None, None, None, None), "Volume and cost")
