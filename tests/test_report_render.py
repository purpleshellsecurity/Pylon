...
def section(document):
    ...
    found = re.search(r'<div class="eyebrow">Diagnostic settings</div>.*?</section>',
                      html, re.S)
    assert found, "the Not logging section did not render"
...
def test_it_renders_at_all(document):
    ...
    for eyebrow in ("Diagnostic settings", "Data sources", "Ingestion gaps"):
...
def test_the_heading_counts_what_the_table_adds_up_to(section):
    ...
    assert "11 of 14 resources, diagnostic setting missing or incomplete" in section
