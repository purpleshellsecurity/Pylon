...
def test_a_solution_row_shows_the_name_and_not_its_publisher():
    ...
    doc = {"workspace": "w", "generated_at": "2026-09-11T00:00:00",
           "window_days": 30, "scope": "/subscriptions/0", "summary": {},
           "reads": {}, "resources": [], "rules": [], "coverage_gaps": [],
           "tables": [], "gaps": []}
    html = build(doc, None, None, None, None, rec)

    assert "Azure Web Application Firewall" in html
    assert "Microsoft Corporation" not in html, "the publisher line is back"
    assert "install" in html, "the action that is about THIS tenant"


def test_the_scan_leaves_one_page_and_the_old_names_stay_ignored():
    """Content Hub is step 6 of the one page now, so nothing writes the second
    file. Both retired names stay ignored: existing checkouts have one on disk."""
    import pathlib

    from pylon.report import OUT

    assert OUT == "telemetry_health_report.html", OUT
    root = pathlib.Path(__file__).resolve().parents[1]
    ignore = (root / ".gitignore").read_text(encoding="utf-8")
    assert OUT in ignore
    for retired in ("content_hub_recommendations.html",
                    "detection_recommendations.html"):
        assert retired in ignore, f"{retired} must stay ignored"

    readme = (root / "README.md").read_text(encoding="utf-8")
    assert OUT in readme
    assert "content_hub_recommendations.html" not in readme, (
        "the README promises a page nothing writes")
