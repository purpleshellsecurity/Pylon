...
# THE CONNECTORS SENTINELHEALTH REPORTS ON, AND ONLY THOSE.
#
# This module already carried the caveat in prose: "Health monitoring covers
# only the connectors Microsoft supports it for, so a connector absent here is
# not necessarily unhealthy." True, and useless to the code, because nothing
# said WHICH. So silence from a connector meant one of two things and the scan
# could not tell them apart:
#
#   a type SentinelHealth never reports on  -> silence proves nothing
#   a covered type that has gone quiet      -> a real finding
#
# Reporting the first as the second is the mistake this whole codebase refuses
# to make, and without the list it was unavoidable.
#
# Verbatim from Microsoft's "Supported data connectors" list, checked
# 2026-09-15 against
# https://learn.microsoft.com/azure/sentinel/monitor-data-connector-health
# (doc last updated 2026-08-08): Amazon Web Services (CloudTrail and S3),
# Dynamics 365, Office 365, Microsoft Defender for Endpoint, Threat
# Intelligence - TAXII, Threat Intelligence Platforms, and any connector based
# on the Codeless Connector Framework.
#
# Spelled as the ARM `kind` values, matched case-insensitively, because that is
# what `crosscheck.connector_kinds` returns.
HEALTH_REPORTS_FOR: frozenset[str] = frozenset({
...
# The Codeless Connector Framework is named as a whole rather than as a list of
# products, so it is matched by the kinds CCF connectors carry. A tenant can
# install any number of these and Microsoft does not enumerate them.
CODELESS_KINDS: frozenset[str] = frozenset({
    "apipolling", "genericui", "restapipoller", "restapipollerv2",
})


def reports_health(kind: str | None) -> bool:
    """Whether SentinelHealth writes rows for this connector kind at all.

    False means silence from this connector is not evidence of anything. The
    caller has to fall back to table freshness rather than call it broken.
    """
    if not kind:
        return False
    k = kind.strip().lower()
    return k in HEALTH_REPORTS_FOR or k in CODELESS_KINDS


def connector_outcomes(kinds: set[str] | None,
                       connectors: dict[str, dict] | None) -> dict[str, list[str]]:
    """Three buckets, never two.

    `kinds` is every connector kind the tenant has, from both legs; `connectors`
    is what SentinelHealth actually reported, keyed by instance.

        healthy      covered, reporting, nothing but Success/Informational
        unhealthy    covered, reporting, and something else
        unreported   NOT covered by health monitoring, so silence means
                     nothing and freshness is the only signal left

    A covered kind with no rows lands in `unreported` too, with the same
    consequence: health monitoring may simply be off. `enabled` on the health
    read is what separates those, and it is the caller's to state.
    """
    out: dict[str, list[str]] = {"healthy": [], "unhealthy": [], "unreported": []}
    seen_kinds: set[str] = set()
    for name, slot in (connectors or {}).items():
        kind = slot.get("kind") or ""
        seen_kinds.add(kind.strip().lower())
        bad = [st for st in (slot.get("statuses") or {})
               if st not in ("Success", "Informational")]
        out["unhealthy" if bad else "healthy"].append(name)
    for kind in sorted(kinds or ()):
        if kind.strip().lower() in seen_kinds:
            continue
        if not reports_health(kind):
            out["unreported"].append(kind)
    for key in out:
        out[key].sort()
    return out
...
def probe(workspace_guid: str, days: int = 30) -> tuple[dict, dict]:
    ...
    # may simply not report health. In the tenant this was measured against,
    # MCAS was configured and emitted nothing to SentinelHealth.
