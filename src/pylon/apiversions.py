...
# above.
#
# Checked 2026-09-09 against the ARM template reference. The provider offers
# exactly two versions, 2017-04-01 and 2017-04-01-preview. The stable one is
# what every Microsoft sample on that page uses, in Bicep, ARM and Terraform
# alike, and the provider is not marked deprecated or superseded. Old and
# correct.
#
# One thing that page gets wrong is worth writing down, because it is the same
# shape as the bug this file was created for. Its LogSettings.category enum
# lists only AuditLogs and SignInLogs, while its own sample on the same page
# enables RiskyUsers, UserRiskEvents, NonInteractiveUserSignInLogs,
# ServicePrincipalSignInLogs and B2CRequestLogs. The documented enum does not
# cover reality.
#
# It does not reach us: `scopes.tenant` reads whatever categories the setting
# returns and never checks them against a list. Had it done so, every Entra
# category added since 2017 would have been silently dropped -- which is
# exactly how the Auxiliary table plan got past the spend gate.
AADIAM = "2017-04-01"
