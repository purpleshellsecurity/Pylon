...
def build(workspace_arm_id: str, live_tables: set[str],
    ...
    for pkg in installed:
        ...
        if n_rules == 0:
            ...
            #
            # Three unknowns with three different fixes, and they used to share
            # one sentence -- a bucket of rows all reading "check by hand" for
            # reasons a reader could not tell apart. Chosen on the alignment
            # VALUE now: this tested `evidence.startswith("nothing here was
            # measured")`, so rewording the message silently reassigned rows to
            # the wrong sentence. This codebase already has that scar, in
            # rulehealth, where matching a phrase in another module's message
            # decided whether a rule was faulty.
            # `unseen` is not unproven. The usage meter RAN and the table
            # received nothing -- that is a measurement and an instruction,
            # and it sat under the same "unproven" chip as two rows where
            # this scan admits it could not look. Splitting the evidence into
            # three and leaving one verdict word meant the split changed
            # nothing a reader scanning the Do column could see.
