...
def _unfed(needed: list[str], emits: dict[str, dict],
    ...
    DIAGNOSTIC SETTING to assess. It knew 14 tables in the tenant this was
    measured against and knows
    ...
    who = (f"{plural(n, 'resource')} across {plural(len(types), 'type')}"
           if len(types) > 1
...
def build(workspace_arm_id: str, live_tables: set[str],
    ...
    for pkg in installed:
        ...
        c = cat.get(cid) or {}
        # The connector ids themselves, not only the tables they resolve to.
        # A solution declaring a connector is this tenant's own evidence that it
        # expects one, which is the only honest basis for reporting a connector
        # as missing. Resolved to tables and then discarded, the evidence was
        # gone by the time anything could use it.
        needs = sorted(dict.fromkeys(declared(pr) or declared(c)))
        types = {t for k in needs for t in conn.get(k, ())}
        ...
        if types and fed:
            ...
            alignment = "unmatched"
            evidence = (f"none of its {plural(n_rules, 'rule template')} could "
                        "be read from the Content Hub")
        ...
        if n_rules == 0:
            ...
            if ships_rules and present and n_enabled < ships_rules:
                ...
                gap = ships_rules - n_enabled
                detail += (f" {gap} of its {plural(ships_rules, 'detection')} "
                           f"{'is' if gap == 1 else 'are'} not switched on.")
            ...
            detail = (f"Installed, current, and {evidence}. None of its "
                      f"{plural(ships_rules, 'detection')} "
                      f"{'is' if ships_rules == 1 else 'are'} switched on.")
        elif ships_rules and n_enabled and n_enabled < ships_rules and present:
            action = "enable"
            gap = ships_rules - n_enabled
            detail = (f"Installed, current, and {evidence}. "
                      f"{gap} of its {plural(ships_rules, 'detection')} "
                      f"{'has' if gap == 1 else 'have'} never been switched on.")
        else:
            action = "none"
            detail = f"Installed, current, and {evidence}."
