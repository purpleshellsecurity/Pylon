...
def sentinel_monitoring(workspace_arm_id: str, health: dict) -> list[dict]:
    ...
    rows = [{
        ...
        "basis": ((f"{len(connectors) - len(unhealthy)} of "
                   f"{plural(len(connectors), 'connector')} healthy: "
                   f"{per_connector}") if on else
    ...
    rows.append({
        ...
        "basis": (f"{plural(health.get('audit_rows', 0), 'audit event')} in the "
                  f"past {days} days{stamp(health.get('audit_newest'))}" if audit_on else
