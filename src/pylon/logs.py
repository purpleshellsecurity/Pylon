...
class _TerminalOnly(logging.Filter):
    """Drops a record that asked not to be printed to a person.

    `extra={"human": False}` means "record this, do not narrate it". The JSONL
    handler carries no such filter, so the event is still on disk: a verb that
    prints a reference page and exits is not worth a timestamp on screen, and
    IS worth a line in the run record that says what was asked for.
    """

    def filter(self, record: logging.LogRecord) -> bool:
        return getattr(record, "human", True)
...
def configure(level: str | None = None, jsonl: Path | None = None) -> Path | None:
    ...
    # A record can ask not to be shown to a person while still being recorded.
    # On the HANDLER for the same reason `run_id_filter` is: a record from a
    # child logger reaches these handlers without passing the logger's filters.
    human.addFilter(_TerminalOnly())
