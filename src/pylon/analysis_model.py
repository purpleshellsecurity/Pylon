...
class Solution(Strict):
    ...
    # unknown     RETIRED. Nothing emits it now -- `unseen`, `unmeasured` and
    #             `unmatched` replaced it, because one word for three different
    #             situations made a bucket of rows that all read "check by hand"
    #             for reasons a reader could not tell apart. Kept in the union
    #             so an analysis.json written before the split still loads.
