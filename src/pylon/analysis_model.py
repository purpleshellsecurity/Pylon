...
The scan document used to be shaped into a dict by a function, which made the
data model implicit in that function: the only way to know what a field may
hold was to read the code that filled it, and the only way to know a run had
produced a valid document was to look at it. This declares the document as
types instead, so the
...
SCHEMA_VERSION = 1
# The recommendations document has its own shape and its own history; it reached
# v2 when the unit became the solution rather than the rule template. It is not
# `Analysis`, and it must not inherit `Analysis`'s number.
...
LEGS: dict[str, str] = {
    ...
    # Connectors as a source. Tied to `differential`, which is the leg that
    # unions both connector legs into the set of kinds this tenant has.
...
class Reads(Strict):
    ...
    # Whether every deployed rule's query could be resolved to the tables it
    # reads. A rule calling a saved KQL function -- an ASIM parser, a Content
    # Hub solution's parser, which Microsoft's own guidance says to write --
    # names no table in its own text, and this scan does not open function
    # bodies. Those rules are absent from the table and coverage sections
    # entirely, so the figures there are a FLOOR.
    #
    # `report.py` has counted this since the `not resolved` label was added and
    # rendered it as a grey note under one table. That put a caveat about two
    # whole sections in the middle of a third, where a reader scanning the
    # unanswered questions would never meet it.
    rule_tables: ReadState = ReadState(ran=False)
...
class Resource(Strict):
    ...
    # Azure's own word for a VM's operating system, from the Resource Graph
    # query that already selects it. None for every type that has no such
    # concept, and for a scan that ran before this field existed.
...
class Connector(Strict):
    """A data connector, judged as a SOURCE rather than as a health signal.

    Microsoft 365, AWS, Defender and threat intel have no diagnostic setting, so
    the connector is the only thing that fills their tables. Read only as health
    it sat at the wrong end of the chain, and a tenant with no Office 365
    connector had nothing saying so until an empty table turned up two steps
    later with no cause attached.

    `state` is three-valued and the third one is why this is safe to publish:

        present         either connector leg saw it
        expected-absent something in THIS tenant expects it and it is not here
        not-established neither leg ran, so nothing can be called absent

    `because` carries the evidence, always. There is no built-in list of
    connectors a tenant ought to have and there must not be: what a client
    should connect depends on what they run and license, and a recommended list
    would be an opinion wearing the costume of a measurement.
    """

    connector: str
    state: Literal["present", "expected-absent", "not-established"]
    because: list[str] = Field(default_factory=list)
    fills: list[str] = Field(default_factory=list)


class BlockedBy(Strict):
    """An upstream break that stops this row being judged on its own merits.

    Carried by rules, techniques and solutions. `step` says which check in the
    chain holds the cause, so a reader knows where to go; `subject` is the thing
    that is broken; `resources` is every resource behind it, because ten storage
    accounts with no diagnostic setting is ONE fix and a count of ten.

    Absent is not the same as fine. A row is stamped only where a break was
    MEASURED upstream. A setting the scan could not read leaves this null and
    the row unknown, because inventing a cause from a question nobody answered
    is the mistake these documents exist to refuse.
    """
...
class RuleHealth(Strict):
    ...
    # Whether it is switched on in the workspace. Read from the rule's own
    # properties, so it is known even on a run where the health leg did not
    # execute -- which is exactly when it matters, because `never-fires` is
    # unavailable and `disabled` still is not.
    #
    # It was carried on the sidecar as `_enabled` and stripped on the way into
    # this document, so the analysis could not tell an enabled rule from a
    # disabled one and the report had to go and read rules_detail.json to find
    # out. The docstring above says "one deployed, ENABLED scheduled rule" and
    # nothing enforced it.
    enabled: bool | None = None
    # Set when every table this rule reads is empty for the same measured
    # reason further upstream. The rule is not misconfigured and retuning it
    # changes nothing.
...
GapType = Literal[
    ...
    "no_content",
    # A technique whose only claiming rules CANNOT fire: disabled in the
    # workspace, or rated never-fires or broken by the health leg that ran two
    # steps earlier. `gapscan` used to ignore both facts and call these
    # covered, which overstated coverage by a third on the tenant this was
    # measured against -- T1078 Valid Accounts among them, claimed by three
    # rules, all of them dead.
    #
    # Distinct from `available_not_deployed`, which is a technique with no rule
    # at all. Here somebody already built the detection and it is not running.
    "rule_cannot_fire",
]
...
class Gap(Strict):
    ...
    # Set when every rule claiming this technique is stopped by the same thing
    # further upstream. The technique is uncovered either way; this says the fix
    # is not here.
    blocked_by: BlockedBy | None = None
...
class SuggestedDetection(Strict):
    ...
    # federation trust settings" is the case in point: the tenant this was
    # measured against has zero federated domains, and the description says
    # it fires on "Update domain
...
class Solution(Strict):
    ...
    # Analytics > Rule templates. In the tenant this was measured against,
    # 306 templates were installed and one rule had been created from them,
    # which no other field says.
    rules_enabled: int = 0
    ...
    # The connector ids this solution declares as dependencies. Kept because a
    # solution declaring a connector is this tenant's own evidence that it
    # expects that source, which is the only honest basis for reporting a
    # connector as missing. Resolved to tables and then discarded, the evidence
    # was gone by the time anything could use it.
...
class Recommendations(Strict):
    ...
    # Versioned SEPARATELY from `Analysis`. These are two documents with two
    # shapes, and sharing one constant meant a v2 recommendations file defaulted
    # to declaring v1 -- latent, because nothing reads it yet, and wrong the
    ...
    # What it cost, and these are MEASURED ZEROS rather than absent figures.
    # This document costs nothing to produce: `contenthub.py` measures the
    # solutions and no model is called. `None` here would mean "nobody looked",
    # which is the one thing that is not true -- the distinction this whole
    # codebase turns on, applied to its own cheapest document.
    #
    # Defaulted rather than passed in because there is exactly one construction
    # site and no way for it to cost anything. If a paid path ever builds one of
    # these, it must pass real figures and `cost_is_partial`.
...
class Analysis(Strict):
    ...
    # Connectors as a SOURCE at step 1, not as a health signal at step 2. Only
    # ones this tenant has or demonstrably expects; see `connectors.py` for why
