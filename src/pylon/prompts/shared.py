...
# NO EXCLUSION SCAFFOLDING RULE, and it must not come back.
#
# It required every detection to open with a `_GetWatchlist('ApprovedAutomation')`
# allowlist and filter the caller against it. Two reasons it is gone.
#
# It is wrong advice on a whole class of detection. Unconditional, so a
# permanent Global Administrator grant outside PIM was told to carry a place to
# suppress principals -- on the one detection where every occurrence is meant to
# be reviewed.
#
# And no query could satisfy it. `_dead_empty_collections` rejects
# `!in (dynamic([]))` and `_unverified_watchlists` rejects the _GetWatchlist
# form, both as errors, so the prompt demanded what the gates refused.
#
# Both gates stay. Tuning advice belongs in each detection's own
# `tuning_guidance`, which can say "narrow by role scope" on a privileged grant
# and "allowlist the deploy principal" on a config write.
