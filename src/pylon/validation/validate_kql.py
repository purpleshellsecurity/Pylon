...
def _known_tables() -> frozenset[str]:
    ...
    silence before the column checks existed) from a real one where no column
    schema is available."""
...
def validate_kql(kql: str, table_name: str, expected_provider: str = "",
    ...
    # Before the column checks existed this was total silence: only
    # known-but-wrong tables warned.
    detected = extract_query_table(kql)
    ...
    # a column reference. Also stops a `|` inside a string from being read
