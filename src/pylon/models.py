...
class LiveCheck(Strict):
    """Result of executing a detection query against a real workspace (opt-in).
...
class OfflineCheck(Strict):
    ...
    (kustainer) with the table declared as an empty typed datatable. ``ran`` =
...
class ValidatedDetection(Strict):
    ...
    warnings: list[str]
    retried: bool
    live_check: LiveCheck | None = None  # populated only when --verify-live runs
    offline_check: OfflineCheck | None = None  # populated only when --verify-offline runs
    tune: TuneResult | None = None  # populated only when --tune runs
