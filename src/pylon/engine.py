...
class EngineRequest:
    ...
    target_key: str = ""
    # Tables the scanned workspace actually has, from `Analysis.provisioned_tables`.
...
async def run_detection_phase(
    ...
    async def generate_one(vector) -> ValidatedDetection:
        ...
        # F6: models sometimes embellish the technique ID it returns
...
async def run_playbook_phase(request: EngineRequest, target: ValidatedDetection) -> str:
    ...
    # Phase 2 validates every detection and retries once. Phase 3 returned the
    # model's text unchecked — and a playbook carries 6-10 KQL queries and 3
    # PowerShell blocks that a responder copy-pastes at 3am. A `let` that shadows
    # the column it is compared against, or a Graph cmdlet issued on an Azure
    # connection, both survived to the assets that way. Same gate, same one
    # corrective retry.
    # Fence any bare query BEFORE checking. Two consecutive runs came back with
    # fourteen unfenced queries each, through a header rule and a corrective
    # retry, so asking does not work. Fencing here makes it a property of the
    # output instead of a request -- and the real checks then run on queries that
    # were previously invisible to every validator.
