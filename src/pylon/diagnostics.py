...
def _probe_one(resource: dict, flow_logs: dict, dcr: dict,
    ...
    return _verdict(rid, rtype, "resource", surfaces,
                    {"subscription": resource.get("subscription"),
                     ...
                     # Carried, not recomputed: `_agent_surface` above already
                     # decided this VM's agent table from it. A machine with no
                     # Defender sensor has no other source for its OS -- there
                     # is nothing onboarded to ask -- so dropping it here left
                     # the report unable to say what the machine was.
