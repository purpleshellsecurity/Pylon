...
#
# DOCS_BASE tracks a branch and its markdown reaches the model's context at run
# time, so upstream can change this tool's output with no signal. A hash keeps
# the content current where a commit pin would freeze it.
#
# NOT a gate: a doc page legitimately changes, and refusing to run over that is
# worse than the drift. One line in the run log.
_SEEN: dict[str, str] = {}
