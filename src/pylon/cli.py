"""Three verbs, and the boundary between them is what each one is allowed to claim.
    ...
    analyze     read the tenant and report it. What is logging, what is
                detecting, what is exposed, and what to switch on in Content
                Hub. Measurement only: every figure comes from something this
                run observed, and a verdict with nothing behind it is the one
                thing the documents refuse to carry.

    design      write what does not exist. Takes a platform and a service and
                generates KQL against a grounded catalogue.

    config      store the settings the other two need, in a file readable only
                by its owner.

There was a fourth, `recommend`, and removing it is worth recording. It was
meant to hold a boundary: analyze reports STATE, recommend reports DECISIONS
about content. The boundary was real and the command was not enforcing it --
`inventory.run` computed the Content Hub decisions during the scan and wrote
them to disk, and `recommend` only rendered that file. No Azure call, no
recompute. So a scan finished having written a document it did not show you,
and the second verb cost a command and bought nothing.

The distinction survives where it belongs, in the document: state and decisions
are different sections, and a decision about content that rests on a dead
source says so rather than being reported as its own problem.
...
lazily. `analyze` must run on a machine with no model provider configured -- a
scan that needs an LLM SDK to say which tables are dark has the wrong shape.
"""
...
def _analyze(args: argparse.Namespace) -> int:
    """Scan, then render the report.

    There was a second page and a second verb to render it. `pylon recommend`
    read nothing -- `inventory.run` computed the Content Hub decisions during
    the scan and wrote them to disk, and the verb only drew them. So a scan
    finished having written a document it did not show you, and the two pages
    were built from the same measurements and shown one at a time.

    Worse, they overlapped. One missing storage diagnostic setting produced a
    finding on the health page AND a "connect the data source" action on the
    Content Hub page, in two vocabularies, neither admitting they were the same
    cause. Content Hub is step 6 of one chain, not a second document.

    `--render-only` is what survives: the page back from the scan already on
    disk, without paying for another one.
    """
    from . import inventory, report

    if not args.render_only:
        # `--workspace` stopped being argparse-required when `--render-only`
        # arrived, so the "neither was given" case is checked here instead of
        # scanning against None.
        if not args.workspace:
            print("pylon analyze: give --workspace to scan, or --render-only "
                  "to redraw the page from the analysis.json already here",
                  file=sys.stderr)
            return 2
...
def _design_list(args: argparse.Namespace) -> int:
    ...
    def detail(target) -> None:
        ...
        print(console.heading(target.key))
        ...
        for table in tables_for(target.key):
            ...
            print(f"      {console.label(table)}"
                  + (f"   ({plane})" if plane else ""))
            print(f"        filter on   {operation_column(table)} {operator} \"...\""
                  + (f"   ({why})" if why else ""))
            if vocab:
                print(f"        grounded on "
                      f"{plural(len(vocab), 'known operation')}, spelled like:")
    ...
    # and BEFORE them, because two streams arriving at one terminal put this
    # after the list, where it read as a footer the command had trailed off
    # into. Flushed, so the ordering holds.
    # Painted for stderr, because that is where it is printed. `label()`
    # defaults to asking stdout, so a redirect of one and not the other put
    # escapes in a file or dropped them from a terminal.
    print(f"  {console.label('pylon design list <value>', sys.stderr)}"
          "         what it queries and the operation strings it grounds "
          "against\n"
          f"  {console.label('pylon design detections <value>', sys.stderr)}"
          "   design detections for it\n", file=sys.stderr)
    sys.stderr.flush()
...
def _design_detections(args: argparse.Namespace) -> int:
    ...
    try:
        report = asyncio.run(engine.runnable().run(request))
...
def _design_tuning(args) -> int:
    ...
    if args.out:
        ...
        return 0
    # Rendered for a terminal, returned untouched for a pipe. `> x.md` and
    # `--out x.md` have to produce the same file.
    print(console.markdown(text))
    return 0
...
def _design_record(args: argparse.Namespace) -> int:
    ...
    for d in detections:
        ...
        # anything else in the same table, which catches a filter that matches
        # the wrong thing or dropped itself.
        #
        ...
        # thing: a detection ending `project TimeGenerated, Scope, ...` hands
        # back only its own columns, so the fixture lacks the fields the query
        # reads and grades silent against rows it selected itself.
        #
        # Positives taken this way cannot prove the query is right -- it will
        # always fire on them. They are a REGRESSION ANCHOR. The evidence is in
        # the negatives: real rows of the same operation it does NOT match.
        #
        # `EventDataId` is the key every detection here dedupes on. A query that
        # does not surface it gets None and falls through, rather than this
        # guessing at another key.
...
def _design_survey(args: argparse.Namespace) -> int:
    ...
    for table in sorted(tables):
        column = operation_column(table)
        # COUNT WHAT A DETECTION CAN REACH, not what happened. On a table with
        # an attribution gate the two differ, and the gap is the whole point:
        # blob storage recorded 95 ListContainers events and NONE of them
        # OAuth, so a detection that gates on the principal -- as the rules
        # require -- can never match one. Survey called that vector gradable,
        # and the money was spent before the zero was visible.
        #
        # `countif(true)` on a table with no gate, so nothing changes there, and
        # it stays one round trip per table rather than two.
        gate = (contracts.for_table(table) or {}).get("attribution", {}).get("gate")
        if gate:
            gates[table] = gate
        rows = validate._run_kql(
            f"{table} | summarize n=count(), reachable=countif({gate or 'true'}) "
            f"by Op=tostring({column})", guid, window)
        if rows is None:
            print(f"  {table}: could not be read, so nothing below is measured "
                  f"for it", file=sys.stderr)
            counts[table] = {}
            continue
        # With no gate the two are the same number by definition, so say so
        # here rather than depending on `countif(true)` coming back.
        counts[table] = {
            str(r.get("Op", "")).lower():
            (int(r.get("n") or 0),
             int(r.get("reachable") or 0) if gate else int(r.get("n") or 0))
            for r in rows}

    gradable, blind = [], []
    for i, v in enumerate(vectors, 1):
        events, reachable = counts.get(v.get("log_table", ""), {}).get(
            (v.get("operation") or "").lower(), (0, 0))
        # Gradable on what a detection can REACH. An operation that happened
        # but never attributably is not something to pay to build.
        (gradable if reachable else blind).append((i, v, events, reachable))

    width = max((len(v.get("name", "")) for _i, v, _n, _r in gradable + blind),
                default=20)
    width = min(max(width, 20), 56)
    # BOTH numbers, because the gap between them is a finding. "95 events, 0
    # reachable" says the operation happens constantly and never attributably,
    # which is worth knowing; a bare 0 reads as a quiet tenant.
    gated = [t for t in sorted(tables) if t in gates]
    head = f"  {'#':>3}  {'vector':{width}}  {'events':>7}"
    print(f"\n{head}  {'reachable':>9}" if gated else f"\n{head}")
    print("  " + "-" * (width + (26 if gated else 16)))
    for i, v, n, r in gradable + blind:
        line = f"  {i:>3}  {v.get('name','')[:width]:{width}}  {n:>7}"
        print(f"{line}  {r:>9}" if gated else line)

    for table, gate in sorted(gates.items()):
        print(f"\n  reachable = rows of {table} where `{gate}`. The other "
              f"rows name nobody, and a detection that reports an actor cannot "
              f"be built from them.", file=sys.stderr)

    print(f"\n  {len(gradable)} of {len(vectors)} can be graded against this "
          f"workspace.", file=sys.stderr)
    if blind:
        happened = sum(1 for _i, _v, n, r in blind if n and not r)
        print(f"  {len(blind)} cannot be. Generating them costs the same and "
              f"returns `no-ground-truth`, which is not a pass -- trigger the "
              f"operation first, or build them knowing they are unproven.",
              file=sys.stderr)
        if happened:
            print(f"  {happened} of those DID happen and were never "
                  f"attributable. That is a fact about the tenant, not a quiet "
                  f"one: the operation runs, and not once by a named principal.",
                  file=sys.stderr)
    if gradable:
        ...
        print(f"\n  pylon design detections --from {args.source} --pick "
              f"\"{','.join(str(i) for i, _v, _n, _r in gradable)}\" "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog="analyze, validate and design work on a tenant. "
    ...
    # What is left of `pylon recommend`. That verb rendered a second page from
    # the same scan and never read the tenant, so it is gone; the two pages are
    # one now, and the capability worth keeping is getting it back without
    # paying for another scan.
    a.add_argument("--render-only", action="store_true",
                   help="re-render the report from the analysis.json already "
                        "on disk, without scanning")
    ...
    dn = d.add_subparsers(
        ...
        description="Four of these spend money and are marked PAID; the other "
                    "seven are free. `list`, `tuning` and `survey` are free on "
                    "purpose -- they are how you decide what is worth paying "
                    "for, and `survey` is how you avoid building detections "
                    "this workspace could never prove.")

    # Registered in WORKFLOW order, not definition order, because argparse prints
    # subcommands in the order they are added and that listing is the first thing
    # a new reader sees. It used to open on `plan` and end on `list` -- so the
    # free command that tells you what targets exist was last of eleven, and the
    # two other free orientation commands were just above it. Decide, build,
    # prove; the free ones that stop you overspending come first.
    ...
    dp_ = dn.add_parser("plan",
                        help="PAID (one model call): what it would build, before it builds it")
    ...
    dd = dn.add_parser("detections", help="PAID (one call per vector -- the bill): threat analysis, then KQL")
    ...
    dp = dn.add_parser("playbooks", help="PAID (one call per playbook): IR playbooks for detections already generated")
    ...
    dsw = dn.add_parser("sweep",
                        help="PAID (every target, one shared budget): plan, build, write "
                             "playbooks for and grade each, resumably")
...
# Verbs that read vendored data, print it, and change nothing. They answer in
# milliseconds, so the HH:MM:SS the log formatter prints is the first thing on
# screen and tells a reader nothing about a command that has already finished.
# Everything that touches Azure, spends money or writes a file keeps it, because
# there the question really is "how long has it been sitting there".
REFERENCE_VERBS = frozenset({_design_list, _design_tuning, _design_coverage,
                             _config})
...
def main() -> int:
    ...
    # A reference page gets no timestamp. Identified by the handler itself
    # rather than by parsing argv: the set cannot drift from the verbs it
    # names, and renaming a subcommand cannot silently put the banner back.
    _cli_log.info("pylon %s", " ".join(sys.argv[1:]) or "(no arguments)",
                  extra={"event": "command",
                         "command": getattr(args, "command", ""),
                         "argv": sys.argv[1:],
                         "human": getattr(args, "func", None) not in REFERENCE_VERBS,
