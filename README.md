...
On a table whose contract carries an attribution gate it prints a second number,
`reachable` — rows a detection that reports an actor could actually match. The
two differ more than you would expect: blob storage recorded 95 `ListContainers`
events in thirty days and none of them under OAuth, so a detection for it can
never fire, and the gap is worth seeing before you pay for one.
...
as an issue. It also says what Pylon touches: it reads and never writes Azure
resources, credentials never reach the command line or the run log, and `design`
is the only command that sends anything to a model provider.

**Hit a bug?** A crash prints the path to its own run log and where to report it.
Attaching that log is the single most useful thing you can include.
