...
#
# The reference pages document more than names and types: some column
# descriptions carry the VALUES the column takes. That is the failure class this
# repo keeps paying for — a wrong value parses, validates, executes and matches
# nothing, so the rule is silently dead. `identity_s` vs `Identity` was the name
# version; `ActivityStatusValue in ("Succeeded")` against a tenant writing
# `Success` is the value version.
#
# Two kinds, and conflating them would manufacture the same bug from the other
# side. "Possible values:" is a CLOSED set and can be enforced. "for example" is
# explicitly illustrative and must never be enforced — measured 2026-08-28, the
# pages list RemoteIPType as "for example Public, Private, Reserved, Loopback,
# Teredo, FourToSixMapping, and Broadcast" and a live workspace returned
# `LinkLocal`, which is in none of them. Enforcing that list would reject a
# correct query about a value the tenant holds right now.
#
# The bold-bullet shape is deliberately NOT parsed. DeviceLogonEvents documents
# LogonType as "- **Interactive**: ... - **Remote interactive (RDP) logons**:
# ..." and those labels are prose, not literals — the column's value is
# `RemoteInteractive`. Harvesting the label would feed a value that can never
# match, which is the bug this whole section exists to prevent. That set is
# worth having and has to be confirmed against a live table first.
